...
async def create_new_thread(user_id, tenant_id, title: str, thread_type: ThreadType, content: List[BlockModel] = [],parent_block_id:str=None):
    try:
       ...
       if not old_thread:
           ...
           print("\n\n 👉 5.a.1 Creating new thread Model",parent_block_id,"\n\n")
           new_thread = ThreadModel(creator_id=creator['id'], title=title, type=thread_type,tenant_id=userinfo['tenant_id'],parent_block_id=parent_block_id)
