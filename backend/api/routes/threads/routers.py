...
async def create(request: Request, thread_title: str, block: CreateBlockModel = Body(...),
    ...
    try:
        ...
        part_1 = time.time()
        print(f"profiling Time elapsed for first part(): { part_1 - start_time:.6f} seconds")
        new_block = await create_new_block(block, user_id, tenant_id)
        #print("profiling performance 2")
        part_2 = time.time()
        print(f"profiling Time elapsed for create_new_block(): {part_2 - part_1:.6f} seconds")
    ...
    #  ret_thread = await get_thread_from_db(thread_id, tenant_id)
        end_time = time.time()
        print(f"profiling Time elapsed for user_flags(): {end_time - part_2:.6f} seconds")
        print(f"profiling Time elapsed for create block(): {end_time - start_time:.6f} seconds")
